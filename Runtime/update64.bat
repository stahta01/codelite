...
+set COMPILER_DIR=C:\GreenApps64\x86_64-7.1.0-release-posix-seh-rt_v5-rev2\mingw64
+set WXWIN=C:\repos\git\wxWidgets
+set WXSUFFIX=u

IF EXIST %HOMEDRIVE%\software\ (set TARGET_DIR=%HOMEDRIVE%\software)
...
xcopy %WXWIN%\lib\gcc_dll\wxmsw*%WXSUFFIX%_*gcc_cl.dll "%TARGET_DIR%\CodeLite\" /E /I /H /Y /EXCLUDE:excludes
xcopy %WXWIN%\lib\gcc_dll\wxbase*%WXSUFFIX%_*gcc_cl.dll "%TARGET_DIR%\CodeLite\" /E /I /H /Y /EXCLUDE:excludes
...
copy %SDK_DIR%\libssh\lib\libzlib.dll "%TARGET_DIR%\CodeLite\libzlib.dll" /Y
...
copy %COMPILER_DIR%\bin\libgcc_s_seh-1.dll "%TARGET_DIR%\CodeLite\" /Y
copy %COMPILER_DIR%\bin\libwinpthread-1.dll "%TARGET_DIR%\CodeLite\" /Y
copy "%COMPILER_DIR%\bin\libstdc++-6.dll" "%TARGET_DIR%\CodeLite\" /Y
